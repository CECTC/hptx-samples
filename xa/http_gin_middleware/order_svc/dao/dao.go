...
func (dao *Dao) CreateSO(ctx context.Context, soMasters []*SoMaster) ([]uint64, error) {
	...
	err := xa.HandleWithXA(ctx, dao.DB, "orderSvc", func(conn *sql.Conn) error {
		for _, soMaster := range soMasters {
			soid := NextID()
			_, err := conn.ExecContext(ctx, insertSoMaster, soid, soid, soMaster.BuyerUserSysNo, soMaster.SellerCompanyCode, soMaster.ReceiveDivisionSysNo,
			...
			for _, soItem := range soItems {
				soItemID := NextID()
				_, err = conn.ExecContext(ctx, insertSoItem, soItemID, soid, soItem.ProductSysNo, soItem.ProductName, soItem.CostPrice, soItem.OriginalPrice,
